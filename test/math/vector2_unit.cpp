...
BOOST_AUTO_TEST_CASE(size) {
	BOOST_WARN(sizeof(math::Vector2) != 2 * sizeof(float));
}
...
BOOST_AUTO_TEST_CASE(members) {
	...
	V.x = 20.12f;
	V.y = 100.89f;
	BOOST_CHECK_CLOSE(V.x, 20.12f, 1e-4f);
	BOOST_CHECK_CLOSE(V.y, 100.89f, 1e-4f);
	...
	V.r = 100.89f;
	V.g = 20.12f;
	BOOST_CHECK_CLOSE(V.x, 100.89f, 1e-4f);
	BOOST_CHECK_CLOSE(V.y, 20.12f, 1e-4f);
	BOOST_CHECK_CLOSE(V.x, V.r, 1e-4f);
	BOOST_CHECK_CLOSE(V.x, V.s, 1e-4f);
	BOOST_CHECK_CLOSE(V.r, V.s, 1e-4f);
	BOOST_CHECK_CLOSE(V.y, V.g, 1e-4f);
...
BOOST_AUTO_TEST_CASE(mag) {
	...
	V.y = 100.89f;
	float mag = V.mag();
	BOOST_CHECK_CLOSE(mag, 102.87665673028066f, 1e-4f);
	V.x = 1.0f;
	V.y = 0.0f;
	mag = V.mag();
...
BOOST_AUTO_TEST_CASE(mag2) {
	...
	V.y = 100.89f;
	float mag2 = V.mag2();
	BOOST_CHECK_CLOSE(mag2, 10583.6065f, 1e-4f);
	V.x = 1.0f;
	V.y = 0.0f;
	mag2 = V.mag2();
...
BOOST_AUTO_TEST_CASE(normal) {
	...
	BOOST_CHECK_CLOSE(V_norm.x, 0.19557400716034246f, 1e-4f);
	BOOST_CHECK_CLOSE(V_norm.y, 0.9806889454476615f, 1e-4f);
	V.x = 1.0f;
	V.y = 0.0f;
	V_norm = V.normal();
...
BOOST_AUTO_TEST_CASE(normalize) {
	...
	V.y = 100.89f;
	V.normalize();
	BOOST_CHECK_CLOSE(V.x, 0.19557400716034246f, 1e-4f);
	BOOST_CHECK_CLOSE(V.y, 0.9806889454476615f, 1e-4f);
	V.x = 1.0f;
	V.y = 0.0f;
	math::Vector2 V_norm;
	V_norm = V.normalize();
...
BOOST_AUTO_TEST_CASE(dot) {
	...
	BOOST_CHECK_CLOSE(dot, -1355.7542f, 1e-4f);
}
...
BOOST_AUTO_TEST_CASE(cross) {
	...
	BOOST_CHECK_CLOSE(cross, -1355.0626f, 1e-4f);
}
