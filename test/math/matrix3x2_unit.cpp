...
BOOST_AUTO_TEST_CASE(size) {
	BOOST_WARN(sizeof(math::Matrix3x2) != 3 * 2 * sizeof(float));
}
...
BOOST_AUTO_TEST_CASE(create) {
	...
	math::Matrix3x2 M_param2(col1, col2, col3);
...
BOOST_AUTO_TEST_CASE(members) {
	...
	BOOST_CHECK_THROW(M[3] = math::Vector2(), std::out_of_range);
}
...
BOOST_AUTO_TEST_CASE(matrix_add) {
	...
	BOOST_CHECK_CLOSE(M_add[2][0], 7.0f, 1e-4f);
	BOOST_CHECK_CLOSE(M_add[2][1], 7.0f, 1e-4f);
}
...
BOOST_AUTO_TEST_CASE(matrix_sub) {
	...
	BOOST_CHECK_CLOSE(M_sub[2][0], 3.0f, 1e-4f);
	BOOST_CHECK_CLOSE(M_sub[2][1], 5.0f, 1e-4f);
}
